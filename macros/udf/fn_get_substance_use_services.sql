{% macro fn_get_substance_use_services() %}

    TRIM(CONCAT('' '', 
            IFF(OUTPATIENT, ''outpatient '', ''''),
            IFF(INTENSIVE_OUTPATIENT_SU_SERVICES, ''intensive_outpatient '', ''''),
            IFF(GENERAL_TREATMENT, ''general_treatment '', ''''),
            IFF(DUI_DWI, ''dui_dwi '', ''''),
            ...
            IFF(contains(DUI_DWAI_DRIVING_UNDER_THE_INFLUENCE_D, ''DUI/DWAI Level II Four Plus, Track F (ASAM Level 1.0)'' ), ''level_ii_four_plus'', ''''),
