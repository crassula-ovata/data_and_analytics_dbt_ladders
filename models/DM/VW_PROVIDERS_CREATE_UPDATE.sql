...
, p_prod as (
    ...
    select * from p_prod_all where date_rank_p = 1 
)
, p_share_union as (
	select * 
	from (
		...
		union
		select external_id parent_account_id, case_name bha_general_acct, 2 as priority from p_prod_all
	)
...
, p_share_union_otp as (
    ...
    from (
        ...
        union
        select external_id parent_account_id, opioid_treatment_provider, 2 as priority from p_prod_all
    )
    qualify row_number() over (partition by parent_account_id order by priority asc) = 1
)
, new_providers as (
...
, updated_providers as (
    select 
        p_prod_all.date_opened as u_date_opened,
        p_prod_all.case_id,
        p_prod_all.external_id,
        locs.location_id as owner_id,
        'provider' as case_type,
        p_prod_all.case_name as case_name,
        iff(p_share_union_otp.opioid_treatment_provider = p_prod_all.opioid_treatment_provider, 
            p_prod_all.opioid_treatment_provider, p_share_union_otp.opioid_treatment_provider) as opioid_treatment_provider,
        iff(p_prod_all.external_id = p_share_union.parent_account_id and 
            p_prod_all.case_name <> p_share_union.bha_general_acct, 'update_name', null) as name_action,
        iff(p_prod_all.external_id = p_share_union.parent_account_id and 
            nvl(p_share_union_otp.opioid_treatment_provider::string,'') <> nvl(p_prod_all.opioid_treatment_provider::string, ''), 
            'update_otp', null) as otp_action,
        iff(p_prod_all.external_id = p_share_union.parent_account_id and 
            nvl(p_prod_all.owner_id,'') <> nvl(locs.location_id, ''), 'update_owner', null) as owner_action,
        iff(name_action = 'update_name' or otp_action = 'update_otp' or owner_action = 'update_owner', 'update', null) as action,
    current_timestamp() as import_date
    from p_prod_all left join p_share_union on  p_prod_all.external_id = p_share_union.parent_account_id
                 left join p_share_union_otp on p_share_union_otp.parent_account_id = p_prod_all.external_id
                 left join locs on locs.site_code = p_prod_all.external_id
